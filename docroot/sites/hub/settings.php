...
if (isset($_GET['q']) && strpos($_GET['q'], 'sites/default/files/styles') === 0) {
...
// require DRUPAL_ROOT . '/sites/all/settings/cdn.settings.php';

// Includes functions for protecting a site via ip, password, etc.
require_once DRUPAL_ROOT . '/sites/hub/settings/permissions.settings.php';

// Includes settings for Acquia Search (via apachesolr).
require DRUPAL_ROOT . '/sites/hub/settings/search.settings.php';
...
if (file_exists(DRUPAL_ROOT . '/sites/hub/settings/local.settings.php')) {
  require DRUPAL_ROOT . '/sites/hub/settings/local.settings.php';
}
...
if (isset($_SERVER['DEVDESKTOP_DRUPAL_SETTINGS_DIR']) && file_exists($_SERVER['DEVDESKTOP_DRUPAL_SETTINGS_DIR'] . '/loc_hub_dd.inc')) {
  require $_SERVER['DEVDESKTOP_DRUPAL_SETTINGS_DIR'] . '/loc_hub_dd.inc';
}
// </DDSETTINGS>

// Include HUB feed settings.
if (isset($_ENV['AH_SITE_ENVIRONMENT']) && file_exists("/mnt/gfs/{$ac_subname}.{$_ENV['AH_SITE_ENVIRONMENT']}/nobackup/hub_feed.settings.php")) {
  require "/mnt/gfs/{$ac_subname}.{$_ENV['AH_SITE_ENVIRONMENT']}/nobackup/hub_feed.settings.php";
}
...
if (isset($_ENV['AH_SITE_ENVIRONMENT']) && $_ENV['AH_SITE_ENVIRONMENT'] == 'prod') {
   ...
   *
   * See README.txt in the fast_404 module directory for more information.
   */
  ...
  $conf['fast_404_path_whitelisting'] = TRUE;

  # Array of whitelisted files/urls. Used if whitelisting is set to TRUE.
  $conf['fast_404_url_whitelist'] = array('index.php', 'rss.xml', 'install.php', 'cron.php', 'update.php', 'xmlrpc.php',
      'api/v1/user-tools-access-records');

  $conf['fast_404_path_whitelist'] = array('index.php', 'rss.xml', 'install.php', 'cron.php', 'update.php', 'xmlrpc.php',
    'api/v1/user-tools-access-records');

  # Array of whitelisted URL fragment strings that conflict with fast_404.
  $conf['fast_404_string_whitelisting'] = array('cdn/farfuture', '/advagg_', 'api/v1');
  ...
  $conf['fast_404_HTML_error_page'] = './hub_404.html';
